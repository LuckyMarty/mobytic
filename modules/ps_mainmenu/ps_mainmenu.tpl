...
{function name="menu" nodes=[] depth=0 parent=null}
  {if $nodes|count}
    <ul class="top-menu" {if $depth == 0}id="top-menu" {/if} data-depth="{$depth}">
      <li class="link">
        <a class="dropdown-item" href="{$urls.base_url}" data-depth="0"><img src="../..{$urls.theme_assets}icons/icon-home.svg"></a>
      </li>
      ...
      {foreach from=$nodes item=node}
        <li class="{$node.type}{if $node.current} current {/if}" id="{$node.page_identifier}">
          ...
          <a class="{if $depth >= 0}dropdown-item{/if}{if $depth === 1} dropdown-submenu{/if}" 
            ...
            href="{if $node.page_identifier == 'lnk-boutique'}{if Group::getCurrent()->id == 4}/5-professionnels{else}{$node.url}{/if}{else}{$node.url}{/if}"
